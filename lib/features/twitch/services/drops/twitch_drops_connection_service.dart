...
import '../../api/drops/twitch_drops_query_presets.dart';
import '../auth/twitch_drops_auth_service.dart';
import 'twitch_drops_connection_check.dart';
import 'twitch_drops_snapshot.dart';

class TwitchDropsCollectResult {
  final bool ok;
  ...
  const TwitchDropsCollectResult({
    required this.ok,
...
class TwitchDropsConnectionService {
  final TwitchApiClient apiClient;
  final TwitchDropsAuthService dropsAuthService;

  const TwitchDropsConnectionService({
    required this.apiClient,
    required this.dropsAuthService,
  });

  Future<TwitchDropsConnectionCheck> checkConnection() async {
    ...
    try {
      ...
      final inventory = await _postGql(
        token: validToken,
        clientId: clientId,
        bodyJson: TwitchDropsQueryPresets.inventoryJson(),
      );

      final campaigns = await _postGql(
        token: validToken,
        clientId: clientId,
        bodyJson: TwitchDropsQueryPresets.campaignsJson(),
      );
      ...
      final snapshot = !inventoryHasErrors && !campaignsHasErrors
          ? TwitchDropsSnapshot.fromResponses(
  ...
  TwitchDropsConnectionCheck _result({
    ...
    return TwitchDropsConnectionCheck(
      hasToken: hasToken,
      tokenValid: tokenValid,
      clientId: clientId,
